...
Methane Watch (Act 2, the mission) adds four data layers, each with its own legend row (**The One Ramp Per Unit Rule:** plasma is ppm·m, viridis is ppb; the two never share a bar, and magma is never used because it reads as plasma's twin). **The watch globe:** when the baseline lands the map switches to MapLibre's globe projection facing the watch areas and turns slowly (4° a second) while the agent works; it stops when a methane raster takes the frame, when the presenter grabs the map, and never starts under `prefers-reduced-motion`. Sites are fog dots (2.5–6 px by dates seen); sites seen on five or more dates carry a 2 px fog ring and a `×N` mono label at 18 px; watch areas are dashed fog hairlines (50%). Dates are a count, not a measurement, so sites are fog, never a data ramp; the legend's sites key draws a dot and a ring. The **TROPOMI** composite renders in viridis over 0–60 ppb at 75% opacity and is hidden above zoom 8, so its ~4 km cells tip the eye without walling off the EMIT finding (the frontend enforces the cap even when a hint omits it). The **EMIT pass window** is plasma, masked below 500 ppm·m so the ground shows through. The **columns** stand one per enhanced pixel, 0.25 m per ppm·m, on the same plasma ramp at 90%, read at a 55° tilt with a −20° bearing; the ground scene that follows keeps the tilt. Clearing the map returns it to a flat, upright Mercator view.
...
A numbered list of the agent's tool calls for the current turn, with exactly one lit marker. Each row: an 8px dot (amber with a 5px amber-wash halo while running, signal green when done) beside a two-digit mono step number, then the tool name in body 15px (17px, weight 600, with a small amber "working" label on the lit step). At most nine steps show; earlier ones collapse to a mono "+n earlier" line at 15px. Consecutive calls of one tool are one step ("Scan 7 watch areas"), with a 15px label-grey line under it naming what it covered (the tool's own watch-area names only) or what it followed ("Following the TROPOMI tip", then the cued point in mono). The plate rises into place (8px translate, 360ms) and never reflows the map.

A Methane Watch turn adds a **plan strip** above the steps (Baseline · Tip · Cue · Brief in the display face at 15px; while working the current stage is amber, done stages fog, the rest label grey) and a **pass filmstrip** under the cue step: one 44px frame per EMIT pass judged, newest first, candidates with a 2px fog frame, rejected passes at 45%, and one 15px line saying the verdicts in words ("5 candidates · 2 too weak · 1 cloud or gap"). A frame opens the evidence plate with the tool's reason. The cue step stays pinned while later steps push earlier ones under "+n earlier".

### Brief card (signature, the end frame)
When a turn drafts a brief, the step column becomes the end frame: the plan strip, the chosen site's filmstrip, then the brief card, with the full step list behind a quiet "Show all n steps". The card's heading leads with its state in label grey ("Draft brief", "Filed brief"; the amber belongs to the decision, and the plan strip turns all fog once the turn is done) and the title in the display face at 19px; then the count sentence (numbers in mono), the two confidence lines ("Methane recurs here", "Any single explanation"), up to three unconfirmed hypotheses with their assessment in label grey, and the decision: "Approve and file" (primary) and "Request another look" (quiet). The backend files the brief; the card then says "Filed by the analyst at … UTC. The agent did not file it." In a replay case the card is read-only and says the decision is made live. Its data comes from the draft in S3 through the backend, never from the model's text.
