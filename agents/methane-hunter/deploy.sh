#!/bin/bash
# Deploy the Methane Hunter to Amazon Bedrock AgentCore.
#
#   DEPLOY_TARGET=dev ./deploy.sh                          # methane_hunter_dev, reads .env.dev
#   DEPLOY_TARGET=stable CONFIRM_STABLE=yes ./deploy.sh    # methane_hunter (live), reads .env
#   DRY_RUN=1 DEPLOY_TARGET=dev ./deploy.sh                # print the commands (secrets redacted)
#
# Uses the same guard as the Earth Analyst (geo_agent/deploy_lib.sh) with this agent's names.
set -e
cd "$(dirname "$0")"

export STABLE_AGENT_NAME="methane_hunter"
export DEV_AGENT_NAME="methane_hunter_dev"
source ../../geo_agent/deploy_lib.sh
resolve_deploy_target

echo "Loading configuration from ${ENV_FILE}..."
set -a
source "${ENV_FILE}"
set +a

required_vars=("S3_BUCKET_NAME" "AGENTCORE_ARN" "ARCGIS_MCP_TOKEN" "EARTHDATA_TOKEN")
for var in "${required_vars[@]}"; do
    if [ -z "${!var}" ]; then
        echo "Error: Required environment variable $var is not set in ${ENV_FILE}"
        exit 1
    fi
done

MODEL_ID="${MODEL_ID:-us.anthropic.claude-sonnet-4-6}"
AWS_REGION="${AWS_REGION:-us-east-1}"
ARCGIS_MCP_URL="${ARCGIS_MCP_URL:-https://esri.geospatial.tfc.aws.dev/hosting/platform/mcp}"
export AWS_REGION
export AWS_DEFAULT_REGION="${AWS_REGION}"

if [ "${DRY_RUN:-0}" = "1" ]; then
    AWS_ACCOUNT_ID="123456789012"
else
    AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
fi
ECR_SHORT_NAME="bedrock-agentcore-${AGENT_NAME}"
ECR_REPO_NAME="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ECR_SHORT_NAME}"

if [ "${DRY_RUN:-0}" = "1" ]; then
    echo "[dry-run] aws ecr describe-repositories --repository-names ${ECR_SHORT_NAME} || aws ecr create-repository --repository-name ${ECR_SHORT_NAME} --image-scanning-configuration scanOnPush=true"
elif aws ecr describe-repositories --repository-names "${ECR_SHORT_NAME}" --region "${AWS_REGION}" >/dev/null 2>&1; then
    echo "ECR repo ${ECR_SHORT_NAME} exists"
else
    echo "Creating ECR repo ${ECR_SHORT_NAME}..."
    aws ecr create-repository --repository-name "${ECR_SHORT_NAME}" --region "${AWS_REGION}" \
        --image-scanning-configuration scanOnPush=true >/dev/null
fi

echo "Configuration:"
echo "   Target: ${DEPLOY_TARGET} (${AGENT_NAME})"
echo "   S3 Bucket: ${S3_BUCKET_NAME}"
echo "   Model: ${MODEL_ID}"
echo "   Region: ${AWS_REGION}"
echo "   ECR Repo: ${ECR_REPO_NAME}"
echo "   Earthdata token: $([ -n "${EARTHDATA_TOKEN}" ] && echo present || echo MISSING)"
echo ""

# Shared platform code + requirements + Dockerfile into this build context (gitignored).
echo "Staging shared platform code..."
run ./stage_shared.sh

echo "Configuring agent ${AGENT_NAME}..."
run "${AGENTCORE_BIN}" configure \
    --deployment-type container \
    --entrypoint methane_hunter.py \
    --name "${AGENT_NAME}" \
    --ecr "${ECR_REPO_NAME}" \
    --execution-role "${AGENTCORE_ARN}" \
    --disable-memory \
    --disable-otel \
    --non-interactive

# `configure` overwrites the Dockerfile; regenerate it from the platform's.
run ./stage_shared.sh dockerfile

echo ""
echo "Launching agent ${AGENT_NAME}..."
run "${AGENTCORE_BIN}" launch \
    --agent "${AGENT_NAME}" \
    --env "AWS_REGION=${AWS_REGION}" \
    --env "S3_BUCKET_NAME=${S3_BUCKET_NAME}" \
    --env "MODEL_ID=${MODEL_ID}" \
    --env "BEDROCK_MODEL_ID=${MODEL_ID}" \
    --env "ARCGIS_MCP_URL=${ARCGIS_MCP_URL}" \
    --env "ARCGIS_MCP_TOKEN=${ARCGIS_MCP_TOKEN}" \
    --env "EARTHDATA_TOKEN=${EARTHDATA_TOKEN}" \
    --env "DISABLE_ADOT_OBSERVABILITY=${DISABLE_ADOT_OBSERVABILITY:-true}" \
    --auto-update-on-conflict

echo ""
echo "Deployment complete! (target: ${DEPLOY_TARGET}, agent: ${AGENT_NAME})"
echo "Next steps:"
echo "   1. ARN: grep -A3 '^  ${AGENT_NAME}:' .bedrock_agentcore.yaml | grep agent_arn"
echo "   2. Confirm env vars persisted on the runtime (EARTHDATA_TOKEN present; never print it)"
echo "   3. Smoke: ../../.venv/bin/python ../../scripts/eval.py --agent methane --target ${DEPLOY_TARGET} --only plumes-permian"
